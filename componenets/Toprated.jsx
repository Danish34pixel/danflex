  ...
  const getTopratedData = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `https://api.themoviedb.org/3/${category}/top_rated?page=${page}`,
        {
          headers: {
      ...
      setTopratedData((prevData) => [...prevData, ...results]);
    ...
    <div className="bg-[#1F1E24] w-full">
      <div className="w-full h-[10vh] flex items-center justify-between px-4 border-b border-zinc-800">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl text-zinc-500 font-bold">Top Rated</h1>
        </div>
        <div className="flex items-center gap-4">
          <TopNav />
          <Dropdown
      ...
      <Verticalcards data={topratedData} lastItemRef={lastItemRef} />
      {isLoading && <Loading />}
